...
def comparator(lst1, lst2):
    ...
    """
    if not len(list(lst1)) == len(list(lst2)):
            return False
    for i in range(len(list(lst1))):
        if lst1[i] < lst2[i]:
                return False
    return True
...
class banker(object):
    ...
    def __init__(self, filepath):
        ...
        with open(filepath, "r+") as f:
            self.n, self.m = [int(a) for a in f.readline().rstrip().split(" ")]
            self.available = [int(c) for c in f.readline().rstrip().split(" ")]
            self.max_req = []
            for i in range(self.m): self.max_req.append([int(c) for c in f.readline().rstrip().split(" ")])
            self.allocated = []
            for i in range(self.m): self.allocated.append([int(c) for c in f.readline().rstrip().split(" ")])
            self.need = [list(map(sub, ma, al)) for ma, al in zip(self.max_req, self.allocated)]
            self.safe_check()
    ...
    def safe_check(self):
        ...
        """
        finish = [False for _ in range(self.m)]
        work = self.available
    ...
    def simulate(self, cid):
        ...
        for i in range(20): #while True: # set to 20 instead of while to terminate for output.txt
            if random.random() > .5:
                # attempt to request
                lock.acquire()
                acquire_target = [random.randrange(0, self.max_req[cid][i]+1) for i in range(len(self.available))]
                ...
                # attempt to release
                lock.acquire()
                release_target = [random.randrange(0, self.allocated[cid][i] + 1) for i in range(len(self.available))]
